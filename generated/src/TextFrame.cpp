...
TextFrame::TextFrame(int width, int height, sf::RenderWindow& window_main, const std::string& buttonType)
    ...
    try
    {
        ...
        if (buttonType == "Open") {
            openButton = ButtonFactory::createButton(ButtonFactory::Open,static_cast<float>(width) / 2 - 100, static_cast<float>(height) - 100, 200, 50,"Open");
        } else if (buttonType == "Save") {
            openButton = ButtonFactory::createButton(ButtonFactory::Save,static_cast<float>(width) / 2 - 100, static_cast<float>(height) - 100, 200, 50, "Save");
        } else if (buttonType == "Saturation") {
            openButton = ButtonFactory::createButton(ButtonFactory::Saturation,static_cast<float>(width) / 2 - 100, static_cast<float>(height) - 100, 200, 50, "Apply");
        } else if (buttonType == "Contrast") {
            openButton = ButtonFactory::createButton(ButtonFactory::Contrast,static_cast<float>(width) / 2 - 100, static_cast<float>(height) - 100, 200, 50, "Apply");
        } else if (buttonType == "Blur") {
            openButton = ButtonFactory::createButton(ButtonFactory::Blur,static_cast<float>(width) / 2 - 100, static_cast<float>(height) - 100, 200, 50, "Apply");
        } else if (buttonType == "Crop") {
            openButton = ButtonFactory::createButton(ButtonFactory::Crop,static_cast<float>(width) / 2 - 100, static_cast<float>(height) - 100, 200, 50, "Apply");
        }
